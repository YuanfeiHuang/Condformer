        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ('block{:d}'.format(i),
                     nn.Sequential(
                         *[bottleneck(min(n_channels * (2 ** (i)), max_chn), min(n_channels * (2 ** (i)) // 2, 64))
...
class Noise_Prior_Estimation(nn.Module):
    def __init__(self, n_colors, patch_size, n_condition):
        super(Noise_Prior_Estimation, self).__init__()
...
class LoNPE(nn.Module):
    def __init__(self, inp_channels=3, num_conditions=2, patch_size=32):
        super(LoNPE, self).__init__()

        self.noise_estimation = Noise_Prior_Estimation(inp_channels, patch_size=patch_size, n_condition=num_conditions)

    def forward(self, inp_img):
        return self.noise_estimation(inp_img)


##########################################################################
##---------- Condformer-----------------------
class Condformer(nn.Module):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        super(Condformer, self).__init__()
        self.dim = dim
    ...
    def forward(self, inp_img, inp_prior):
        ...
        latent = self.latent({'x': inp_enc_level4, 'z': inp_prior})['x']
        ...
        return out_dec_level1, inp_prior
...
class Restormer(nn.Module):
    def __init__(self,
        ...
        # self.patch_embed_latent = OverlapPatchEmbed(int(dim * 2 ** 3) + dim*2, int(dim * 2 ** 3))
